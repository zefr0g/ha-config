...
module shell() {
    difference() {
        union() {
            // Outer organic shell minus inner hollow
            difference() {
                outer_body();
                inner_hollow();
            }
            ...
            // Pillar cylinders are at [±44,±38] — always inside outer_body for z≥0,
            // so they are added unconditionally (no intersection needed).
            // Rib endpoints reach x=±57 and y=±52/−52.5 which are outside outer_body
            // below z≈rs=8, so the rib hulls are clipped by intersection(outer_body).
            for (pt = pillar_pts)
                ...
                union()
                    for (pt = pillar_pts) {
                        ...
                        hull() {
                            translate([pt[0], pt[1], 0])
                                cylinder(h = pillar_h, d = pillar_od);
                            translate([iwall_x - sx*0.5, pt[1]-1.5, 0])
                                cube([1, 3, pillar_h]);
                        }
                        hull() {
                            translate([pt[0], pt[1], 0])
                                cylinder(h = pillar_h, d = pillar_od);
                            translate([pt[0]-1.5, iwall_y - sy*0.5, 0])
                                cube([3, 1, pillar_h]);
                        }
                    }
            }
        ...
        // ── Insert holes in pillars (from shell bottom face up)
        for (pt = pillar_pts)
            translate([pt[0], pt[1], 0])
...
module plate() {
    plate_t = 3;
    // Symmetric 8-sphere hull: top 4 spheres at z=+rs (same as outer_body bottom spheres),
    // bottom 4 at z=-rs (mirror). Sliced to z=-plate_t..0.
    // Both top and bottom cross-sections are the full case profile (118×108) — no taper.
    // Top face is flat at z=0; bottom corners get gentle organic rounding.
    difference() {
        intersection() {
            hull()
                for (sx=[-1,1], sy=[-1,1]) {
                    translate([sx*(case_w/2-rs), sy*(case_d/2-rs),  rs]) sphere(r=rs);
                    translate([sx*(case_w/2-rs), sy*(case_d/2-rs), -rs]) sphere(r=rs);
                }
            translate([0, 0, -plate_t/2])
                cube([case_w+2, case_d+2, plate_t], center=true);
        }
        // M2.5 clearance through plate + countersink at plate bottom face
        for (pt = pillar_pts) {
            translate([pt[0], pt[1], -plate_t-1])
                cylinder(h = plate_t+2, d = mount_d);
            translate([pt[0], pt[1], -plate_t])
                cylinder(h = mount_head_h, d1=mount_head_d, d2=mount_d);
        }
    }
}
